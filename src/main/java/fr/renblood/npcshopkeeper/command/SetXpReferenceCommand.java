                                        ...
                                        .then(Commands.argument("max", FloatArgumentType.floatArg(0))
                                                .executes(context -> {
                                                    ...
                                                    if (!JOBS.contains(job)) {
                                                        context.getSource().sendFailure(Component.literal("Erreur : Métier inconnu '" + job + "'."));
                                                        return 0;
                                                    }

                                                    if (min > max) {
                                                        context.getSource().sendFailure(Component.literal("Erreur : Min doit être inférieur ou égal à Max."));
                                                        return 0;
                                                    }
                                                    ...
                                                    if (success) {
                                                        context.getSource().sendSuccess(() -> Component.literal("Référence XP définie pour " + itemName + " : Job=" + job + ", XP=" + min + "-" + max), true);
                                                        return 1;
                                                    } else {
                                                        context.getSource().sendFailure(Component.literal("Erreur lors de l'enregistrement de la référence XP."));
                                                        return 0;
                                                    }
                                                })
                                        )
